...
import { FROM_ALONE_CREATE_VOTE_PLACE, ROOM_TYPE_ALONE } from '@/constants';
...
export default function CreateAlone() {
        ...
        if (existence) {
          ...
          try {
            ...
            if (error.response && error.response.status === 401) {
              navigate(`/page/login/${roomId}`, { state: { from: FROM_ALONE_CREATE_VOTE_PLACE } });
            }
            if (error.response && error.response.status === 403) {
              navigate(`/page/alone/${roomId}`, { state: { from: FROM_ALONE_CREATE_VOTE_PLACE } });
            }
